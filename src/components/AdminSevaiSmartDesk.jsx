...
import {
  ...
  FileCheck2, Ticket, User, CreditCard, Globe, QrCode, Calendar, Clock, X, Layers
} from 'lucide-react';
...
export default function AdminSevaiSmartDesk({ notify, changeAdminPassword, initialVaultTab = 'all' }) {
  ...
  const [activeVaultTab, setActiveVaultTab] = useState(initialVaultTab === 'documents' ? 'documents' : 'tokens');
  const [deskTab, setDeskTab] = useState(initialVaultTab || 'all'); // 'all', 'receipt', 'voice', 'ads', 'vault', 'documents'

  useEffect(() => {
    if (initialVaultTab && initialVaultTab !== 'all') {
      setDeskTab(initialVaultTab);
      setActiveVaultTab(initialVaultTab);
    }
  }, [initialVaultTab]);
  ...
  return (
    <div id="admin-smartdesk-console" style={{ background: '#f8fafc', border: '2px solid #0052cc', borderRadius: '18px', padding: '28px', textAlign: 'left', margin: '20px 0' }}>
      ...
      <div ref={moduleTabsRef} className="smartdesk-module-tabs" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', margin: '16px 0 24px', background: '#ffffff', padding: '10px 14px', borderRadius: '14px', border: '1px solid #cbd5e1', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        ...
        <button
          type="button"
          onClick={(e) => { setDeskTab('documents'); setActiveVaultTab('documents'); e.currentTarget.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); }}
          style={{
            padding: '8px 14px',
            borderRadius: '10px',
            fontSize: '12px',
            ...
            border: 'none',
            cursor: 'pointer',
            background: deskTab === 'documents' ? '#16a34a' : '#f1f5f9',
            color: deskTab === 'documents' ? 'white' : '#475569',
            display: 'inline-flex',
            alignItems: 'center',
            gap: '6px'
          }}
        >
          📁 ஆவணக் காப்பகம் (Documents Vault)
          {customerDocs.length > 0 && (
            <span style={{ background: deskTab === 'documents' ? '#ffffff' : '#16a34a', color: deskTab === 'documents' ? '#166534' : '#ffffff', padding: '2px 7px', borderRadius: '10px', fontSize: '10px', fontWeight: 900 }}>
              {customerDocs.length}
            </span>
          )}
        </button>
