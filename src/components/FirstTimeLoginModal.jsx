...
import { Sparkles, ShieldCheck, Zap, Headphones, ArrowRight, PartyPopper, Lock } from 'lucide-react';
...
export default function FirstTimeLoginModal({ isOpen, customerName, isFirstTime = true, onClose }) {
  ...
  return (
    <div className="logout-modal-backdrop" onClick={onClose}>
      <div
        className="logout-modal-card welcome-first-login-card"
        onClick={(e) => e.stopPropagation()}
        style={{
          borderColor: isFirstTime ? '#f59e0b' : '#16a34a',
          boxShadow: isFirstTime
            ? '0 30px 70px rgba(245, 158, 11, 0.25)'
            : '0 30px 70px rgba(22, 163, 74, 0.25)'
        }}
      >
        {/* ICON CONTAINER */}
        <div
          className="first-login-icon-circle"
          style={{ background: isFirstTime ? '#fef3c7' : '#dcfce7' }}
        >
        ...
        {/* KICKER BADGE */}
        <span
          className="logout-kicker"
          style={{
            background: isFirstTime ? '#fef3c7' : '#dcfce7',
            color: isFirstTime ? '#b45309' : '#15803d',
            borderColor: isFirstTime ? '#fde68a' : '#86efac'
          }}
        >
        ...
        <div
          style={{
            background: isFirstTime ? '#fffbeb' : '#f0fdf4',
            border: `1.5px solid ${isFirstTime ? '#fde68a' : '#86efac'}`,
            borderRadius: '12px',
            padding: '14px',
            margin: '14px 0',
            textAlign: 'left'
